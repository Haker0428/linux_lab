...
struct globalmem_dev *g_globalmem_devp;

static int globalmem_open(struct inode *inode, struct file *filp)
{
	filp->private_data = g_globalmem_devp;
	return 0;
}
...
{
	int err;
	dev_t dev_no = MKDEV(globalmem_major, index);
...
static int __init globalmem_init(void)
{
	...
	g_globalmem_devp = kzalloc(sizeof(struct globalmem_dev), GFP_KERNEL);
	if (!g_globalmem_devp) {
		ret = -ENOMEM;
		goto fail_malloc;
	}
...
{
	cdev_del(&g_globalmem_devp->cdev);
